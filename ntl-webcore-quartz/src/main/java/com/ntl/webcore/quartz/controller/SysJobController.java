package com.ntl.webcore.quartz.controller;
...
import com.ntl.webcore.common.web.exception.job.TaskException;
import com.ntl.webcore.common.web.model.AjaxResult;
import com.ntl.webcore.common.web.utils.ApplicationPropertiesUtil;
import com.ntl.webcore.framework.page.domain.TableDataInfo;
import com.ntl.webcore.quartz.entity.SysJob;
import com.ntl.webcore.quartz.service.ISysJobService;
import com.ntl.webcore.quartz.util.CronUtils;
import com.ntl.webcore.quartz.util.ScheduleUtils;
...
public class SysJobController extends BaseController
{
    ...
    public AjaxResult addSave(@Validated SysJob job) throws SchedulerException, TaskException
    {
        ...
        else if (StrUtils.containsIgnoreCase(job.getInvokeTarget(), Constants.LOOKUP_RMI))
        {
            return error("新增任务'" + job.getJobName() + "'失败，目标字符串不允许'rmi'调用");
        }
        else if (StrUtils.containsAnyIgnoreCase(job.getInvokeTarget(), new String[] { Constants.LOOKUP_LDAP, Constants.LOOKUP_LDAPS }))
        {
            return error("新增任务'" + job.getJobName() + "'失败，目标字符串不允许'ldap(s)'调用");
        }
        else if (StrUtils.containsAnyIgnoreCase(job.getInvokeTarget(), new String[] { Constants.HTTP, Constants.HTTPS }))
        {
            return error("新增任务'" + job.getJobName() + "'失败，目标字符串不允许'http(s)'调用");
        }
        else if (StrUtils.containsAnyIgnoreCase(job.getInvokeTarget(), ApplicationPropertiesUtil.getJobErrorArray()))
        {
