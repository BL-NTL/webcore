...
$(function() {
    ...
    function setActiveTab(element) {
        if (!$(element).hasClass('active')) {
            ...
            // 显示tab对应的内容区
            $('.NTL-Webcore_iframe').each(function() {
    ...
    function menuItem() {
        ...
        $('.menuTab').each(function() {
            if ($(this).data('id') == dataUrl) {
                if (!$(this).hasClass('active')) {
                    ...
                    scrollToTab(this);
                    // 显示tab对应的内容区
                    $('.mainContent .NTL-Webcore_iframe').each(function() {
                        if ($(this).data('id') == dataUrl) {
                            $(this).show().siblings('.NTL-Webcore_iframe').hide();
                            return false;
                        }
        ...
        if (flag) {
            ...
            // 添加选项卡对应的iframe
            var str1 = '<iframe class="NTL-Webcore_iframe" name="iframe' + dataIndex + '" width="100%" height="100%" src="' + dataUrl + '" frameborder="0" data-id="' + dataUrl + '" data-refresh="' + isRefresh + '" seamless></iframe>';
            $('.mainContent').find('iframe.NTL-Webcore_iframe').hide().parents('.mainContent').append(str1);

            $.modal.loading(i18n('index.message.loading'));
    ...
    function refreshTab() {
    	...
    	var target = $('.NTL-Webcore_iframe[data-id="' + currentId + '"]');
    ...
    function tabCloseOther() {
        $('.page-tabs-content').children("[data-id]").not(":first").not(".active").each(function() {
            $('.NTL-Webcore_iframe[data-id="' + $(this).data('id') + '"]').remove();
            $(this).remove();
        });
    ...
    function tabCloseAll() {
        ...
        $('.page-tabs-content').children("[data-id]:first").each(function() {
            $('.NTL-Webcore_iframe[data-id="' + $(this).data('id') + '"]').show();
    ...
    $.contextMenu({
        ...
        items: {
            "close_current": {
                name: i18n('index.contextMenu.closeCurrent'),
                icon: "fa-close",
            ...
            "close_other": {
                name: i18n('index.contextMenu.closeOther'),
            ...
            "close_left": {
                name: i18n('index.contextMenu.closeLeft'),
                icon: "fa-reply",
                callback: function(key, opt) {
                    setActiveTab(this);
            ...
            "close_right": {
                name: i18n('index.contextMenu.closeRight'),
                icon: "fa-share",
                callback: function(key, opt) {
                    setActiveTab(this);
            ...
            "close_all": {
                name: i18n('index.contextMenu.closeAll'),
            ...
            "step": "---------",
            "full": {
                name: i18n('index.contextMenu.showAll'),
                icon: "fa-arrows-alt",
                callback: function(key, opt) {
                    setActiveTab(this);
                    var target = $('.NTL-Webcore_iframe[data-id="' + this.data('id') + '"]');
                    target.fullScreen(true);
                }
            },
            "refresh": {
                name: i18n('index.contextMenu.refresh'),
                icon: "fa-refresh",
                callback: function(key, opt) {
                    setActiveTab(this);
                    var target = $('.NTL-Webcore_iframe[data-id="' + this.data('id') + '"]');
            ...
            "open": {
                name: i18n('index.contextMenu.open'),
                icon: "fa-link",
                callback: function(key, opt) {
                    var target = $('.NTL-Webcore_iframe[data-id="' + this.data('id') + '"]');
