...
var refreshItem = function(){
    ...
    var target = $('.NTL-Webcore_iframe[data-id="' + currentId + '"]', topWindow);
...
var closeItem = function(dataId){
	...
	if($.common.isNotEmpty(dataId)){
	    ...
	    // 移除相应tab对应的内容区
	    $('.mainContent .NTL-Webcore_iframe[data-id="' + dataId + '"]', topWindow).remove();
	    return;
	}
	...
	if($.common.isNotEmpty(panelUrl)){
	    ...
	    $('.mainContent .NTL-Webcore_iframe', topWindow).each(function() {
	        if ($(this).data('id') == panelUrl) {
	            $(this).show().siblings('.NTL-Webcore_iframe').hide();
	            return false;
            }
        });
    }
...
function createMenuItem(dataUrl, menuName, isRefresh) {
    ...
    if (flag) {
        ...
        // 添加选项卡对应的iframe
        var str1 = '<iframe class="NTL-Webcore_iframe" name="iframe' + dataIndex + '" width="100%" height="100%" src="' + dataUrl + '" frameborder="0" data-id="' + dataUrl + '" data-panel="' + panelUrl + '" seamless></iframe>';
        $('.mainContent', topWindow).find('iframe.NTL-Webcore_iframe').hide().parents('.mainContent').append(str1);
        
        window.parent.$.modal.loading(i18n('common.createMenuItem.loading'));
...
function activeWindow() {
    ...
    return $('.NTL-Webcore_iframe[data-id="' + currentId + '"]', topWindow)[0].contentWindow;
}
...
function checkpwd(chrtype, password) {
    if (chrtype == 1) {
        if(!$.common.numValid(password)){
            $.modal.alertWarning(i18n('common.checkpwd.chrtype1.warning'));
            return false;
        }
    } else if (chrtype == 2) {
        if(!$.common.enValid(password)){
            $.modal.alertWarning(i18n('common.checkpwd.chrtype2.warning'));
            return false;
        }
    } else if (chrtype == 3) {
        if(!$.common.enNumValid(password)){
            $.modal.alertWarning(i18n('common.checkpwd.chrtype3.warning'));
            return false;
        }
    } else if (chrtype == 4) {
        if(!$.common.charValid(password)){
            $.modal.alertWarning(i18n('common.checkpwd.chrtype4.warning'));
            return false;
        }
    }
    return true;
...
var sub = {
    ...
    delRow: function(column) {
        ...
        if (ids.length == 0) {
            $.modal.alertWarning(i18n('common.sub.delRow.warning'));
            return;
        }
...
$.ajaxSetup({
    complete: function(XMLHttpRequest, textStatus) {
        if (textStatus == 'timeout') {
            $.modal.alertWarning(i18n('common.ajaxSetup.timeout.warning'));
            $.modal.enable();
            $.modal.closeLoading();
        } else if (textStatus == "parsererror" || textStatus == "error") {
            $.modal.alertWarning(i18n('common.ajaxSetup.error.warning'));
            $.modal.enable();
            $.modal.closeLoading();
        }
    }
