    ...
    $.extend({
        ...
        table: {
                ...
                $(optionsIds).on("click", '.tooltip-show', function() {
                    ...
                    if ($.common.equals("copy", target)) {
                        ...
                        top.layer.alert(input.val(), {
                            title: i18n("ui.table.initEvent.tooltip.title"),
                            shadeClose: true,
                            btn: [i18n('ui.table.initEvent.tooltip.btn')],
            ...
            exportExcel: function(formId) {
                table.set();
                $.modal.confirm(i18n("ui.table.exportExcel.export.confirmation", table.options.modalName) , function() {
                    var currentId = $.common.isEmpty(formId) ? $('form').attr('id') : formId;
                    var params = $("#" + table.options.id).bootstrapTable('getOptions');
                    var dataParam = $("#" + currentId).serializeArray();
                    dataParam.push({ "name": "orderByColumn", "value": params.sortName });
                    dataParam.push({ "name": "isAsc", "value": params.sortOrder });
                    $.modal.loading(i18n("ui.table.exportExcel.export.loading"));
            ...
            importExcel: function(formId, width, height) {
                ...
                top.layer.open({
                    ...
                    maxmin: true,
                    shade: 0.3,
                    title: i18n('ui.table.importExcel.title', table.options.modalName) ,
                    content: $('#' + currentId).html(),
                    btn: ['<i class="fa fa-check"></i> ' + i18n('ui.table.importExcel.btn.confirm'), '<i class="fa fa-remove"></i> '+ i18n('ui.table.importExcel.btn.cancel')],
                    // 弹层外区域关闭
                    shadeClose: true,
                    btn1: function(index, layero){
                        var file = layero.find('#file').val();
                        if (file == '' || (!$.common.endWith(file, '.xls') && !$.common.endWith(file, '.xlsx'))){
                            $.modal.msgWarning(i18n('ui.table.importExcel.warning'));
                            return false;
                        }
            ...
            // 回显数据字典
            selectDictButton: function(datas, value, methodHtml, className) {
                if ($.common.isEmpty(datas) || $.common.isEmpty(value)) {
                    return '';
                }
                var actions = [];
                $.each(datas, function(index, dict) {
                    if (dict.dictValue == ('' + value)) {
                        ...
                        if(className!= undefined){
                            ...
                            listClass = $.common.equals("default", dict.listClass) || $.common.isEmpty(dict.listClass) ? "badge" : "badge badge-" + dict.listClass;
                        }

                        actions.push($.common.sprintf('<a class="btn btn-rounded %s" onclick="%s">%s</a>', listClass,methodHtml, dict.dictLabel));
                        return false;
                    }
                });
                if (actions.length === 0) {
        ...
        modal: {
            ...
            alert: function(content, type) {
                top.layer.alert(content, {
                    ...
                    title: i18n('ui.modal.alert.title'),
                    btn: [i18n('ui.modal.alert.btn.confirm')],
                    btnclass: ['btn btn-primary'],
                });
            },
            // 错误提示
            ...
            confirm: function (content, callBack) {
                top.layer.confirm(content, {
                    icon: 3,
                    title: i18n('ui.modal.confirm.title'),
                    btn: [i18n('ui.modal.confirm.btn.confirm'), i18n('ui.modal.confirm.btn.cancel')]
                }, function (index) {
                    $.modal.close(index);
                    callBack(true);
                });
            ...
            open: function (title, url, width, height, callback) {
                ...
                top.layer.open({
                    ...
                    title: title,
                    content: url,
                    btn: [i18n('ui.modal.open.btn.confirm'), i18n('ui.modal.open.btn.close')],
                    // 弹层外区域关闭
                    shadeClose: true,
                    yes: callback,
            ...
            openOptions: function (options) {
                ...
                var _title = $.common.isEmpty(options.title) ? i18n('ui.modal.openOptions.systemWindow') : options.title;
                var _width = $.common.isEmpty(options.width) ? "800" : options.width; 
                var _height = $.common.isEmpty(options.height) ? ($(window).height() - 50) : options.height;
                var _btn = ['<i class="fa fa-check"></i> ' + i18n('ui.modal.openOptions.btn.confirm'), '<i class="fa fa-close"></i> ' + i18n('ui.modal.openOptions.btn.close')];
                // 如果是移动端，就使用自适应大小弹窗
                if ($.common.isMobile()) {
                    _width = 'auto';
                    _height = 'auto';
            ...
            openFull: function (title, url, width, height) {
                ...
                var index = top.layer.open({
                    ...
                    title: title,
                    content: url,
                    btn: [i18n('ui.modal.openFull.btn.confirm'), i18n('ui.modal.openFull.btn.close')],
                    // 弹层外区域关闭
                    shadeClose: true,
        ...
        operate: {
            ...
            submit: function(url, type, dataType, data, callback) {
                var config = {
                    ...
                    data: data,
                    beforeSend: function () {
                        $.modal.loading(i18n('ui.operate.submit.loading'));
                    },
                    success: function(result) {
            ...
            detail: function(id, width, height) {
                ...
                var options = {
                    title: table.options.modalName + i18n('ui.operate.detail.title'),
                    width: width,
                    height: height,
                    url: _url,
                    skin: 'layui-layer-gray', 
                    btn: [i18n('ui.operate.detail.btn.close')],
                    yes: function (index, layero) {
                        $.modal.close(index);
                    }
                };
            ...
            detailTab: function(id) {
                table.set();
                $.modal.openTab(i18n('ui.operate.detailTab.title') + table.options.modalName, $.operate.detailUrl(id));
            },
            // 详细访问地址
            detailUrl: function(id) {
                ...
                if ($.common.isNotEmpty(id)) {
                    ...
                    if (id.length == 0) {
                        $.modal.alertWarning(i18n('ui.operate.detailUrl.warning'));
                        return;
                    }
                    url = table.options.detailUrl.replace("{id}", id);
                }
            ...
            remove: function(id) {
                table.set();
                $.modal.confirm(i18n('ui.operate.remove.confirm.title', table.options.modalName) , function() {
            ...
            removeAll: function() {
                ...
                if (rows.length == 0) {
                    $.modal.alertWarning(i18n('ui.operate.removeAll.warning'));
                    return;
                }
                $.modal.confirm(i18n('ui.operate.removeAll.confirm.title', rows.length), function() {
            ...
            clean: function() {
                table.set();
                $.modal.confirm(i18n('ui.operate.clean.confirm.title', table.options.modalName), function() {
            ...
            add: function(id) {
                table.set();
                $.modal.open(i18n('ui.operate.add.title') + table.options.modalName, $.operate.addUrl(id));
            },
            // 添加信息，以tab页展现
            addTab: function (id) {
                table.set();
                $.modal.openTab(i18n('ui.operate.add.title') + table.options.modalName, $.operate.addUrl(id));
            },
            // 添加信息 全屏
            addFull: function(id) {
                table.set();
                $.modal.openFull(i18n('ui.operate.addFull.title') + table.options.modalName, $.operate.addUrl(id));
            },
            // 添加访问地址
            addUrl: function(id) {
            ...
            edit: function(id) {
                ...
                if ($.common.isEmpty(id) && table.options.type == table_type.bootstrapTreeTable) {
                    ...
                    if ($.common.isEmpty(row)) {
                        $.modal.alertWarning(i18n('ui.operate.edit.warning'));
                        return;
                    }
                    var url = table.options.updateUrl.replace("{id}", row[table.options.uniqueId]);
                    $.modal.open(i18n('ui.operate.edit.title') + table.options.modalName, url);
                } else {
                    $.modal.open(i18n('ui.operate.edit.title') + table.options.modalName, $.operate.editUrl(id));
                }
            },
            // 修改信息，以tab页展现
            editTab: function(id) {
                table.set();
                $.modal.openTab(i18n('ui.operate.edit.title') + table.options.modalName, $.operate.editUrl(id));
            },
            // 修改信息 全屏
            editFull: function(id) {
                ...
                $.modal.openFull(i18n('ui.operate.edit.title') + table.options.modalName, url);
            },
            // 修改访问地址
            editUrl: function(id) {
                var url = "/404.html";
            ...
            save: function(url, data, callback) {
                var config = {
                    ...
                    data: data,
                    beforeSend: function () {
                        $.modal.loading(i18n('ui.operate.save.loading'));
                        $.modal.disable();
                    },
                    success: function(result) {
                        if (typeof callback == "function") {
            ...
            successTabCallback: function(result) {
                if (result.code == web_status.SUCCESS) {
                    ...
                    var topWindow = $('.NTL-Webcore_iframe[data-id="' + currentId + '"]', topWindow)[0];
        ...
        tree: {
            ...
            notAllowParents: function(_tree) {
                ...
                if (nodes.length == 0){
                    $.modal.msgError(i18n('ui.tree.notAllowParents.emptyNode.modal.msg'));
                    return false;
                }
                for (var i = 0; i < nodes.length; i++) {
                    if (nodes[i].level == 0) {
                        $.modal.msgError( i18n('ui.tree.notAllowParents.disallowRootNode.modal.msg', nodes[i].name ));
                        return false;
                    }
                    if (nodes[i].isParent) {
                       $.modal.msgError( i18n('ui.tree.notAllowParents.disallowParentNode.modal.msg', nodes[i].name ));
                        return false;
                    }
                }
                return true;
            ...
            notAllowLastLevel: function(_tree) {
                ...
                for (var i = 0; i < nodes.length; i++) {
                    if (!nodes[i].isParent) {
                        $.modal.msgError( i18n('ui.tree.notAllowLastLevel.disallowLastNode.modal.msg', nodes[i].name ));
                        return false;
                    }
                }
                return true;
        ...
        common: {
            ...
            //转换半角单引号
            escapeSingleQuote: function(title){
